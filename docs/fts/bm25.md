...
- It has **tuning knobs** - You can adjust it for different use cases with `k1` for key word saturation and `b` for document length. (MySQL does not allow us to change the settings, but it is useful to know this as we may have a dat.abase that does either instead of MySQL or as an additional content database)
...
![various k1](../images/fts/various-k1.png)
...
```
For a given number of terms, if b=1 then shorter documents get a higher score relative to a larger document. Rmemeber, it is relative scores not absolute values.

![various b](../images/fts/various-b.png)
